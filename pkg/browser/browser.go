...
func (browser *Browser) Startup() error {
	...
	browser.TaskCtx, browser.taskCancel = chromedp.NewContext(browser.allocCtx, chromedp.WithLogf(browser.log.Debug().Msgf))
...
func (browser *Browser) Tasks(tasks chromedp.Tasks) error {
	...
	go func() {
		...
		if err := chromedp.Run(browser.TaskCtx, tasks); err != nil {
			browser.log.Err(err).Msgf("error running task: %v", err)
			c1 <- false
			return
		}
		c1 <- true
...
func (browser *Browser) Run() error {
	...
	go func() {
		...
		if err := chromedp.Run(browser.TaskCtx); err != nil {
			browser.log.Err(err).Msgf("cannot start chrome: %v", err)
			c1 <- false
			return
		}
		c1 <- true
...
func (browser *Browser) MouseClick(waitVisible string, x, y int64, element string, timeout time.Duration) error {
	...
	go func() {
		...
		if err := chromedp.Run(ctx, actions...); err != nil {
			browser.log.Err(err).Msgf("error running mouseclick: %v", err)
			c1 <- false
			return
		}
		c1 <- true
